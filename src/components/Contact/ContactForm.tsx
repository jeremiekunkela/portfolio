...
const ContactForm: React.FC<ContactFormProps> = ({ isVisible }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!rateLimitCheck.canSubmit) {
      ...
      setRateLimitError(
        `Vous avez atteint la limite d'envoi. Veuillez patienter ${remainingTime} avant de renvoyer un message.`
      );
      return;
    }
  ...
  return (
    <div
      className={`${styles.formContainer} ${isVisible ? styles.visible : ""}`}
    >
      {submitSuccess && (
